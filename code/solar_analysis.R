# Re-analysis of solar power development overlap/impacts with important conservation areas
...
s_n_eu <- terra::rast("data/global_wind_solar_conservation_priorities_2022/1km/n.europe_solar_1km.tif")
m_m_ea <- terra::rast("data/global_wind_solar_conservation_priorities_2022/1km/middle.east_mask_1km.tif")
s_m_ea <- terra::rast("data/global_wind_solar_conservation_priorities_2022/1km/middle.east_solar_1km.tif")
# plot(w_n_eu)
s_glob <- terra::rast("data/global_wind_solar_conservation_priorities_2022/1km/global_solar_1km.tif")
# plot(w_glob)

# pa
pa <- terra::rast("data/global_wind_solar_conservation_priorities_2022/1km/pa_priority_exp_1km.tif")
# plot(pa)
pa_n_eu <- oneimpact::rescale_vals(terra::crop(pa, m_n_eu) * m_n_eu * s_n_eu/s_n_eu)
pa_m_ea <- oneimpact::rescale_vals(terra::crop(pa, m_m_ea) * m_m_ea * s_m_ea/s_m_ea)
# pa_m_ea2 <- oneimpact::rescale_vals(terra::crop(pa, m_m_ea) * m_m_ea)
# pa_m_ea == pa_m_ea2
# plot(pa_m_ea)
...
# in a single go
input <- c(pa_n_eu, s_n_eu)
input <- c(pa_m_ea, s_m_ea)
# plot(input)
...
# solar
solar_index <- list.files("data/global_wind_solar_conservation_priorities_2022/1km/",
                         pattern = "solar") %>% 
  grep(pattern = "global|priority|bivmap|solarmat", invert = TRUE)

solar <- list.files("data/global_wind_solar_conservation_priorities_2022/1km/", 
                   pattern = "solar", full.names = T)[solar_index] %>% 
  sapply(terra::rast)
names(solar) <- regions
...
# here we are being conservative, we could disconsider all the part of the countries
# with high latitudes
# plot(pa[[2]])
...
over_ratio_solar <- parms %>% 
  purrr::pmap(
    function(x, y, z, ...) {
      print(paste(x, y, z))
      input <- c(pa[[x]], solar[[x]])
      or <- input %>% 
...
df_overlap_solar <- parms %>%
  tibble::as_tibble() %>% 
  dplyr::rename(region = 1, pa_threshold = 2, solar_threshold = 3) %>% 
  dplyr::mutate(region_lab = factor(region, levels = regions, labels = regions_labels),
                ...
                solar_th = 1-solar_threshold,
                or_original = purrr::map_dbl(over_ratio_solar, ~ .[[1]]))

# save(df_overlap_solar, file = "output/df_overlap_solar_original.rda")
# write.csv(df_overlap_solar, file = "output/df_overlap_solar_original.csv", row.names = F)
...
# plot
df_overlap_solar %>% 
  ggplot(aes(solar_th, or_original, color = region_lab)) +
  geom_line(size = 1.5) +
  facet_wrap(~pa_th) +
...
over1km_orig <- purrr::map_dbl(over_ratio_solar_1km, ~ .[[1]])
over1km_zoi <- purrr::map_dbl(over_ratio_solar_1km, ~ .[[2]])
over1km_orig_zoi <- purrr::map_dbl(over_ratio_solar_1km, ~ .[[3]])
...
# check
all(df_overlap_solar$or_original == over1km_orig) # TRUE, ok!

# data.frame
df_overlap_solar_1km <- df_overlap_solar %>%
  dplyr::mutate(or_zoi_1km = over1km_zoi,
                or_original_zoi_1km = over1km_orig_zoi)

# save(df_overlap_solar_1km, file = "output/df_overlap_solar_original_zoi1km.rda")
# write.csv(df_overlap_solar_1km, file = "output/df_overlap_solar_original_zoi1km.csv", row.names = F)

# plot
df_overlap_solar_1km %>% 
  ggplot(aes(solar_th, or_original_zoi_1km, color = region_lab)) +
  geom_line(size = 1.2) +
  facet_wrap(~pa_th) +
  scale_x_continuous(labels = scales::percent) +
  labs(x = "Percentage of land devoted to renewables",
...
df_overlap_solar_1km %>% 
  ggplot(aes(solar_th, or_original_zoi_1km/or_original, color = region_lab)) +
...
df_overlap_solar_1km %>% 
  ...
  ggplot(aes(solar_th, or, color = region_lab)) +
  geom_line(size = 1.2) +
...
over5km_orig <- purrr::map_dbl(over_ratio_solar_5km, ~ .[[1]])
over5km_zoi <- purrr::map_dbl(over_ratio_solar_5km, ~ .[[2]])
over5km_orig_zoi <- purrr::map_dbl(over_ratio_solar_5km, ~ .[[3]])

# read original data.frame
load("output/df_overlap_solar_original_zoi1km.rda")
df_overlap_solar_1km

# check
all(df_overlap_solar_1km$or_original == over5km_orig) # TRUE, ok!

# data.frame
df_overlap_solar_5km <- df_overlap_solar_1km %>%
  dplyr::mutate(or_zoi_5km = over5km_zoi,
                or_original_zoi_5km = over5km_orig_zoi)

# save(df_overlap_solar_5km, file = "output/df_overlap_solar_original_zoi5km.rda")
# write.csv(df_overlap_solar_5km, file = "output/df_overlap_solar_original_zoi5km.csv", row.names = F)
load("output/df_overlap_solar_original_zoi5km.rda")

# plot
df_overlap_solar_5km %>% 
  ggplot(aes(solar_th, or_original_zoi_5km, color = region_lab)) +
  geom_line(size = 1.2) +
  facet_wrap(~pa_th) +
  scale_x_continuous(labels = scales::percent) +
  labs(x = "Percentage of land devoted to renewables",
...
df_overlap_solar_5km %>% 
  ggplot(aes(solar_th, or_original_zoi_5km/or_original, color = region_lab)) +
  # ggplot(aes(wind_th, or_zoi_1km, color = region_lab)) +
  geom_line(size = 1.2) +
  facet_wrap(~pa_th) +
  scale_x_continuous(labels = scales::percent) +
...
cols <- c("#5F4690", "#1D6996", "#7DC4C4",
          "#4AA37E", "#73AF48", "#EEB41F",
          "#E79535", "#CE5948", "#6D6D6D")
solar_zoi_1_5 <- df_overlap_solar_5km %>% 
...
ggsave("overlap_ratio_zoi_1_5_solar.png", plot = solar_zoi_1_5, path = "output",
...
# buffer 1km
zoi <- 1000 # in meters
map_solar_1km <- purrr::map(seq_along(wind),
                      function(i) {
                        print(paste(i))
                        input <- c(pa[[i]], wind[[i]])
                        or <- input %>% 
...
map_solar_1km_merge <- terra::merge(map_solar_1km[[1]], map_solar_1km[[2]], map_solar_1km[[3]],
                                    map_solar_1km[[4]], map_solar_1km[[5]], map_solar_1km[[6]],
                                    map_solar_1km[[7]], map_solar_1km[[8]], map_solar_1km[[9]])
plot(map_solar_1km_merge)
map_solar_1km_merge46 <- terra::classify(map_solar_1km_merge, rbind(c(5, 4), c(7, 6)))
freq(map_solar_1km_merge46) %>% tibble::as_tibble() %>% mutate(prop = count/sum(count))

# buffer
zoi <- 5000 # in meters
map_solar_5km <- purrr::map(seq_along(solar),
                      function(i) {
                        print(paste(i))
                        input <- c(pa[[i]], solar[[i]])
                        or <- input %>% 
                          binarize(breaks = list(0.7, 0.7)) %>% 
                          zoi_around(zoi = zoi) %>% 
                          overlap_pa_infra(infra_zoi = 3) 
...
map_solar_5km_merge <- terra::merge(map_solar_5km[[1]], map_solar_5km[[2]], map_solar_5km[[3]],
                                    map_solar_5km[[4]], map_solar_5km[[5]], map_solar_5km[[6]],
                                    map_solar_5km[[7]], map_solar_5km[[8]], map_solar_5km[[9]])
names(map_solar_5km_merge) <- "solar_pa_overlap"
plot(map_solar_5km_merge)
freq(map_solar_5km_merge) %>% tibble::as_tibble() %>% mutate(prop = count/sum(count))
# map_5km_moll = terra::project(map_5km_merge, "+proj=moll")
# plot(map_5km_moll)

# export
terra::writeRaster(map_solar_1km_merge, filename = "output/map_solar_1km_4overout_6overinpa.tif")
terra::writeRaster(map_solar_5km_merge, filename = "output/map_solar_5km_5overout_7overinpa.tif", overwrite = TRUE)

# read
map_solar_5km_merge <- terra::rast("output/map_solar_5km_5overout_7overinpa.tif")*1
...
plot(map_solar_5km_merge, col = cols5)

cols1 <- c("grey", "yellow", "dodgerblue1", "violetred4", "seagreen2", NA, "orange")
plot(map_solar_1km_merge, col = cols1)

# plot with tmap
map_solar_5km_moll <- terra::project(map_solar_5km_merge, "+proj=moll", method = "near")
plot(map_solar_5km_moll, col = cols5)
map_solar_5km_eck4 <- terra::project(map_solar_5km_merge, terra::crs(pa_global), method = "near")
plot(map_solar_5km_eck4, col = cols5)

# zoom maps
m1 <- terra::crop(map_solar_5km_eck4, zoom_areas[1,])
plot(m1, col = cols5)
m2 <- terra::crop(map_solar_5km_eck4, zoom_areas[2,])
plot(m2, col = cols5)
m3 <- terra::crop(map_solar_5km_eck4, zoom_areas[3,])
plot(m3, col = cols5)
m4 <- terra::crop(map_solar_5km_eck4, zoom_areas[4,])
plot(m4, col = cols5)
# m5 <- terra::crop(map_solar_5km_eck4, zoom_areas[5,])
# plot(m5, col = cols5)
...
map_solar_5km_exp <- tm_shape(world_eck4, bbox = st_bbox(world_eck4) + add)  +
  tm_fill(col = grey(0.9)) +
  tm_shape(map_solar_5km_eck4) +
...
tmap_save(map_solar_5km_exp, filename = "output/map_solar_5km_tmap.png", 
...
map_solar_5km_zoom1 <- tm_shape(world_eck4, bbox = zoom_areas[1,]) +
...
map_solar_5km_zoom2 <- tm_shape(world_detailed, bbox = zoom_areas[2,]) +
  tm_polygons(col = grey(0.9)) +
  # tm_shape(map_5km_eck4) +
  tm_shape(m2, ) +
  tm_raster(title = "", style = "fixed", breaks = brk, 
...
map_solar_5km_zoom3 <- tm_shape(world_detailed, bbox = zoom_areas[3,]) +
  tm_polygons(col = grey(0.9)) +
  # tm_shape(map_5km_eck4) +
  tm_shape(m3) +
  tm_raster(title = "", style = "fixed", breaks = brk, 
...
map_solar_5km_zoom4 <- tm_shape(world_detailed, bbox = zoom_areas[4,]) +
  tm_polygons(col = grey(0.9)) +
  # tm_shape(map_5km_eck4) +
  tm_shape(m4) +
  tm_raster(title = "", style = "fixed", breaks = brk, 
...
tmap_save(map_solar_5km_exp, filename = "output/map_solar_5km_tmap.png", 
          ...
          insets_tm = list(map_solar_5km_zoom1, map_solar_5km_zoom2, 
                           map_solar_5km_zoom3, map_solar_5km_zoom4), 
          insets_vp = list(vp1, vp2, vp3, vp4))
